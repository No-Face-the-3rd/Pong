...
void drawMenu(float screenH,float screenW);
...
int main()
{
	...
	drawMenu((float)screenH,(float)screenW);
...
void drawMenu(float screenH, float screenW)
{
	float gap = 5.0f,size=screenH/20.0f;
	while (sfw::stepContext())
	{
		...
		drawChar(2, screenW / 2.0f + 0.0f * (size / 2.0f + gap) + size / 2.0f, size + screenH*2.0f / 3.0f, size);
		drawChar('P', screenW / 2.0f + 1.0f * (size / 2.0f + gap) + size / 2.0f, size + screenH*2.0f / 3.0f, size);
